...
function endpoints(exampleDomain) {
  return [
    { id: 'dynamic-dns', group: 'Dynamic DNS', method: 'POST', path: '/api/v1/dynamic-dns', title: 'Update a dynamic address', description: 'Updates existing A or AAAA records. The token must have dns:dynamic permission and an exact hostname restriction.', auth: true, staff: false, limit: '60 requests/minute per token', parameters: [{ name: 'hostname', required: true, type: 'string', description: 'Exact hostname included in the token restrictions.' }, { name: 'use_request_ip', required: false, type: 'boolean', description: 'Use the public address making this API request.' }, { name: 'ip', required: false, type: 'IPv4/IPv6', description: 'Explicit public address; its family selects A or AAAA.' }, { name: 'ipv4', required: false, type: 'IPv4', description: 'Explicit public IPv4 address.' }, { name: 'ipv6', required: false, type: 'IPv6', description: 'Explicit public IPv6 address.' }], example: (base, key) => `curl -X POST ${base}/api/v1/dynamic-dns \
  -H "Authorization: Bearer ${key}" \
  -H "Content-Type: application/json" \
  -d '{"hostname":"home.${exampleDomain}","use_request_ip":true}'` },
    { id: 'create-analytics', group: 'Analytics', method: 'POST', path: '/api/v1/analytics', title: 'Create analytics', description: 'Enables privacy-friendly analytics for an owned subdomain and returns its website ID and tracking code. Requires analytics:write.', auth: true, staff: false, limit: '30 requests/minute per token', parameters: [{ name: 'subdomain', required: true, type: 'hostname', description: 'Fully qualified subdomain owned by the token user.' }], example: (base, key) => `curl -X POST ${base}/api/v1/analytics \
  -H "Authorization: Bearer ${key}" \
  -H "Content-Type: application/json" \
  -d '{"subdomain":"myproject.${exampleDomain}"}'` },
    { id: 'analytics-tracking-code', group: 'Analytics', method: 'GET', path: '/api/v1/analytics/{subdomain}/tracking-code', title: 'Get tracking code', description: 'Returns the tracker URL and ready-to-paste HTML script for enabled analytics. Requires analytics:read.', auth: true, staff: false, limit: '120 requests/minute per token', parameters: [{ name: 'subdomain', required: true, type: 'path · hostname', description: 'Fully qualified subdomain owned by the token user.' }], example: (base, key) => `curl ${base}/api/v1/analytics/myproject.${exampleDomain}/tracking-code \
  -H "Authorization: Bearer ${key}"` },
    { id: 'analytics-stats', group: 'Analytics', method: 'GET', path: '/api/v1/analytics/{subdomain}/stats', title: 'Get analytics stats', description: 'Returns summary totals, traffic history, active visitors, popular pages, referrers, countries and devices. Requires analytics:read.', auth: true, staff: false, limit: '120 requests/minute per token', parameters: [{ name: 'subdomain', required: true, type: 'path · hostname', description: 'Fully qualified subdomain owned by the token user.' }, { name: 'days', required: false, type: 'integer', description: 'Reporting window from 1 to 365 days; defaults to 30.' }, { name: 'timezone', required: false, type: 'IANA timezone', description: 'Timezone used to group traffic history; defaults to UTC.' }], example: (base, key) => `curl "${base}/api/v1/analytics/myproject.${exampleDomain}/stats?days=30&timezone=Europe%2FLondon" \
  -H "Authorization: Bearer ${key}"` },
